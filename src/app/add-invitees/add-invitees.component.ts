...
this.route.queryParams.subscribe(params => {
 ...
 search = (text$: Observable<string>) =>
  ...
  handleSuccessfulResponse(response)
			...
			if(response.service == "getUsers"){
				if(response.results.length > 0){
					...
					for(let i=0;i<response.results.length;i++){
						...
						if(response.results[i].username != sessionStorage.getItem('username') && !this.validateInvitee(name)){
			this.names.push({name:response.results[i].firstName + " " + response.results[i].lastName, username:response.results[i].username});
		}
		}
				}
				...
				if(response.results.length > 0){
					for(let i=0;i<response.results.length;i++){
						...
						this.invitees.push({name:name,username:username});
					}
				}
	...
	viewEvent(){
		this.router.navigate(["viewevent"], { queryParams: { eventId: this.eventId } });
	}
}